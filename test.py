...
# Load 2: Load CIM model
# Load 3: Load Global model
# Predictions of Global model, CIM model, later fusion, and decision fusion
#----------------------
...
import build_dataset
# modify
test_dataset_cim = build_dataset.dataset_for_cim(label_test, feature_v_test, word_predicts_test, decision_classes_topk_test, decision_words_test)
test_loader_cim = torch.utils.data.DataLoader(test_dataset_cim, batch_size=100, shuffle=False, **kwargs)

# load CIM
opt.method='cim'
model_cim = build_model.build(CUDA,opt)
# modify model name
model_cim = build_model.get_updateModel(model_cim, model_save_path + 'model_cim.pt')
...
#--------------------------------------Predictions of Global model, CIM, later fusion, and decision fusion ----------------------------------------
# dataset
test_loader_cim = torch.utils.data.DataLoader(test_dataset_cim, batch_size=100, shuffle=False, **kwargs)
test_loader_global = torch.utils.data.DataLoader(test_dataset_global, batch_size=100, shuffle=False, **kwargs)
feature_global, output_global, label_global = test_functions.generate_feature_global(test_loader_global, model_global, opt)
feature_cim, output_cim, label_cim = test_functions.generate_feature_cim(test_loader_cim, model_cim, opt)

test_dataset_fusion = build_dataset.dataset_for_fusion(feature_global, feature_cim, label_global)
...
top_k_pre = 5
label = label_global
pre_cim = torch.topk(output_cim, top_k_pre)[1]
...
for i in range(label.shape[0]):
    ...
    for k in range(top_k_pre):
        if k==0:
            print('Top-{} Class prediction: Global model: [{}], HAF: [{}], CIM: [{}] | final prediction: [{}]'.format(k+1, pre_global[i][k], pre_haf[i][k].long(), pre_cim[i][k], pre_refine[i][k]))
        else:
            print('Top-{} Class prediction: Global model: [{}], HAF: [{}], CIM: [{}]'.format(k+1, pre_global[i][k], pre_haf[i][k].long(), pre_cim[i][k]))