...
# Step 2: Training CIM model for fusing outputs of ARL and HAF
# Step 3: Training the global model (visual backbone)
# Step 4: Later fusion of cross-modal features (from CIM module) and glbal visual features (for global model).
#----------------------
...
#------------------------------------Step 2: Training CIM module for fusing outputs of ARL and HAF----------------------------------------------------
# dataset
...
import build_dataset
train_dataset_cim = build_dataset.dataset_for_cim(label_train, feature_v_train, word_predicts_train, decision_classes_topk_train, decision_words_train)
train_loader_cim = torch.utils.data.DataLoader(train_dataset_cim, batch_size=opt.batch_size, shuffle=True, **kwargs)

# CIM
opt.method='cim'
model_cim = build_model.build(CUDA,opt)
optimizer_cim = train_functions.set_optimizer(model_cim,opt)

# raining

for epoch in range(1, EPOCHS + 1):
    train_functions.lr_scheduler(epoch, optimizer_cim, opt.lr_decay, opt.lrd_rate)
    train_functions.train_cim(epoch, train_loader_cim, model_cim, optimizer_cim, opt)

torch.save(model_cim.state_dict(), model_save_path + 'model_cim.pt')
...
#---------------------------------------------------------------Step 4: Later fusion----------------------------------------------------------------------
# dataset
train_loader_cim = torch.utils.data.DataLoader(train_dataset_cim, batch_size=100, shuffle=False, **kwargs)
train_loader_global = torch.utils.data.DataLoader(train_dataset_global, batch_size=100, shuffle=False, **kwargs)
feature_cim, label_cim = train_functions.generate_feature_cim(train_loader_cim, model_cim)
feature_global, label_global = train_functions.generate_feature_global(train_loader_global, model_global)

train_dataset_fusion = build_dataset.dataset_for_fusion(feature_global, feature_cim, label_global)
